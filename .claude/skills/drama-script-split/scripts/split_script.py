...
def segment(paras: list[str]) -> dict:
    ...
    title = paras[first(lambda t: t.strip().startswith("《"))] if first(lambda t: t.strip().startswith("《")) is not None else ""
    subtitle = next((paras[i] for i in nonempty if paras[i].strip().startswith("——")), "")
...
def main():
    ...
    print(f"标题: {info['title'] or '(无)'} | 副标题: {info['subtitle'] or '(无)'}")
    ...
    misc = 2 + (1 if info['summary'] else 0) + len(info['summary']) + (1 if info['style'] else 0) + len(info['style'])
    print(f"正文合计 {body_total} 行 | 非空总计 {info['nonempty']}（差额为标题/标记/时间码等 front matter）")
    ...
    ov = [info["title"], info["subtitle"], "", "【故事梗概】", *info["summary"],
    ...
    for ep in eps:
        ...
        lines = [f"{info['title']} 第{ep['n']}集 · {ep['short']}",
